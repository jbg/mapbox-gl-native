...
void Source::Impl::updateTiles(const UpdateParameters& parameters) {
    ...
    if (overscaledZoom >= zoomRange.min) {
        ...
        if (parameters.fixedPrefetchZoom >= 0) {
            panZoom = std::max<int32_t>(std::min(parameters.fixedPrefetchZoom, idealZoom), zoomRange.min);
        } else if (parameters.dynamicPrefetchZoomDelta >= 0) {
            panZoom = std::max<int32_t>(idealZoom - parameters.dynamicPrefetchZoomDelta, zoomRange.min);
        }
